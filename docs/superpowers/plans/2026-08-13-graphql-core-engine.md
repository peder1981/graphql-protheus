...
- **`custom/backoffice/graphql/config/graphql-config.json` must be deployed to the AppServer's `RootPath` separately from the compiled RPO** — verified against the live server: `compile.sh`/`deploy-rpo.sh` only ever handle `.tlpp` sources compiled into `build/custom.rpo`; a loose `.json` resource file is never part of that RPO and `deploy-rpo.sh` does not copy it anywhere. Without it physically present on the server filesystem, `GqlConfig:new()`'s `MemoRead()` finds nothing, silently falls back to its empty built-in deny-list, and **every table becomes visible** — confirmed live: `SRA`/`SR5` (both explicitly deny-listed) appeared in `__schema.types` until fixed. **`MemoRead()` resolves a bare relative path against `appserver.ini`'s `[P12] RootPath=` (`/protheus12/protheus_data` in this environment), not `SourcePath` and not the AppServer process's OS working directory** — confirmed by targeted edit-and-observe testing against four candidate locations (`SourcePath`, the process cwd, OS root, and `RootPath`); only the `RootPath`-relative copy affected runtime behavior, the other three were silently ignored. Deploy target: `<RootPath>/custom/backoffice/graphql/config/graphql-config.json`. This is a deployment step, not a code defect — `GqlConfig`'s graceful fallback-to-defaults behavior when the file is missing is intentional and stays as designed (spec: config loading). Task 10's README/deployment docs must call this out explicitly as a required manual step distinct from running `deploy-rpo.sh`, or the deny-list silently does nothing in production.
